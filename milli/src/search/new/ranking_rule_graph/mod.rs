...
use super::query_term::QueryTermSubset;
...
pub struct ComputedCondition {
    ...
    pub subsets: Vec<QueryTermSubset>,
}
...
pub struct RankingRuleGraph<G: RankingRuleGraphTrait> {
    ...
    pub nodes_of_condition: MappedInterner<G::Condition, SmallBitmap<QueryNode>>,
}
impl<G: RankingRuleGraphTrait> Clone for RankingRuleGraph<G> {
    fn clone(&self) -> Self {
        Self {
            ...
            nodes_of_condition: self.nodes_of_condition.clone(),
        }
    }
}
