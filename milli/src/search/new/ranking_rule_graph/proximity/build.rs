...
pub fn build_edges(
    ...
    let (left_term_subset, left_end_position) = match &from_node.data {
        QueryNodeData::Term(LocatedQueryTermSubset {
            term_subset: left_term_subset,
            positions: left_positions,
        }) => (left_term_subset, *left_positions.end()),
        ...
        QueryNodeData::Start => {
            return Ok(vec![(
                ...
                Some(
                    conditions_interner
                        .insert(ProximityCondition::Term { term: right_term_subset.clone() }),
                ),
            )])
        }
    ...
    for cost in right_ngram_length..(7 + right_ngram_length) {
        ...
        conditions.push((
            ...
            Some(conditions_interner.insert(ProximityCondition::Uninit {
                left_term: left_term_subset.clone(),
                right_term: right_term_subset.clone(),
